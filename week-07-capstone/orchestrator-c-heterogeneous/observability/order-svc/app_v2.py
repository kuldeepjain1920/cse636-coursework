...
CPU_GAUGE = Gauge(
    ...
    ["version"],  # lets canary_controller.py distinguish v1 vs v2 pods via PromQL
)
REQUEST_COUNTER = Counter(
    "order_svc_requests_total",
    "Total /order requests processed, labeled by outcome",
    ["status", "version"],  # "200"/"500" -- lets PromQL compute error rate; version -- lets it compute per-version error rate
)
...
def order():
    ...
    with tracer.start_as_current_span("order.process") as span:
        ...
        CPU_GAUGE.labels(version=VERSION).set(cpu_pct)  # Prometheus: always reflects the most recent reading
        ...
        if cpu_pct > CPU_ERROR_THRESHOLD:
            ...
            if random.random() < fail_probability:
                ...
                REQUEST_COUNTER.labels(status="500", version=VERSION).inc()
                REQUEST_DURATION.labels(version=VERSION).observe(latency_ms / 1000.0)
                return jsonify({
        ...
        REQUEST_COUNTER.labels(status="200", version=VERSION).inc()
        REQUEST_DURATION.labels(version=VERSION).observe(latency_ms / 1000.0)
...
def metrics_prometheus():
    ...
    CPU_GAUGE.labels(version=VERSION).set(_process.cpu_percent(interval=0.1))
